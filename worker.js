  ...
  async fetch(request, env) {
    ...
    if (url.pathname.endsWith('/diagram')) {
      ...
      try {
        ...
        const diagramPrompt = `Analyse this AI answer and decide the diagram type:
Use "flowchart" if the answer describes STEPS, a PROCESS, a SEQUENCE, or HOW TO DO something (numbered steps, configure, set up, first/then/next/finally language).
Use "mindmap" for CONCEPTS, FEATURES, OPTIONS, or WHAT something is (definitions, capabilities, comparisons).

Return ONLY valid JSON, no markdown:

Flowchart format: {"type":"flowchart","steps":[{"id":"s1","label":"short verb phrase 4 words max","detail":"1-2 sentence explanation","code":"optional path or snippet"}]}
Rules for flowchart: 3-7 steps in sequence. Each label is an imperative verb phrase (e.g. "Navigate to settings", "Click Create Policy"). Steps must be in order.

Mindmap format: {"type":"mindmap","center":"topic max 4 words","nodes":[{"id":"n1","label":"max 3 words","detail":"1-2 sentences","code":"optional"}],"edges":[{"from":"center","to":"n1"}]}
Rules for mindmap: 3-6 nodes. Labels max 3 words.
